...
tf.flags.DEFINE_string("attribute_pickle_file","/tmp/attribute.pkl","Attibute pickle file.")
...
ImageMetadata=namedtuple("ImageMetadata",["image_id","filename","captions","attribute"])
...
def _to_sequence_example(image,decoder,vocab):
 ...
 try:
  ...
  return
 context=tf.train.Features(feature={"image/image_id":_int64_feature(image.image_id),"image/data":_bytes_feature(encoded_image),"image/attribute":_float_feature(image.attribute),})
...
def _process_dataset(name,images,vocab,num_shards):
 images=[ImageMetadata(image.image_id,image.filename,[caption],image.attribute)for image in images for caption in image.captions]
 random.seed(12345)
...
def _load_and_process_metadata(captions_file,image_dir):
 ...
 with open(FLAGS.attribute_pickle_file,'r')as f:
  attribute_data=cPickle.load(f)
 ...
 for image_id,base_filename in id_to_filename:
  ...
  image_metadata.append(ImageMetadata(image_id,filename,captions,attribute_data[base_filename]))

...
class ShowAndTellModel(object):
 ...
 def build_inputs(self):
  if self.mode=="inference":
   ...
   attribute_feed=tf.placeholder(dtype=tf.float32,shape=[1220],name="attribute_feed")
   input_feed=tf.placeholder(dtype=tf.int64,shape=[None],name="input_feed")
   images=tf.expand_dims(self.process_image(image_feed),0)
   attribute=tf.expand_dims(attribute_feed,0)
   ...
   images_attribute_and_captions=[]
   for thread_id in range(self.config.num_preprocess_threads):
    serialized_sequence_example=input_queue.dequeue()
    encoded_image,caption,attribute=input_ops.parse_sequence_example(serialized_sequence_example,image_feature=self.config.image_feature_name,attribute_feature=self.config.attribute_feature_name,caption_feature=self.config.caption_feature_name)
    image=self.process_image(encoded_image,thread_id=thread_id)
    images_attribute_and_captions.append([image,attribute,caption])
   queue_capacity=(2*self.config.num_preprocess_threads*self.config.batch_size)
   images,attribute,input_seqs,target_seqs,input_mask=(input_ops.batch_with_dynamic_pad(images_attribute_and_captions,batch_size=self.config.batch_size,queue_capacity=queue_capacity))
  self.images=images
 ...
 def build_keyword_embeddings(self):
  with tf.variable_scope("hidden_state_attribute_embedding")as scope:
   hidden_state_attribute_embeddings=tf.contrib.layers.fully_connected(inputs=self.attribute,num_outputs=self.config.embedding_size,activation_fn=None,weights_initializer=self.initializer,biases_initializer=None,scope=scope)
  with tf.variable_scope("cell_state_attribute_embedding")as scope:
   cell_state_attribute_embeddings=tf.contrib.layers.fully_connected(inputs=self.attribute,num_outputs=self.config.embedding_size,activation_fn=None,weights_initializer=self.initializer,biases_initializer=None,scope=scope)
  self.hidden_state_attribute_embeddings=hidden_state_attribute_embeddings
  self.cell_state_attribute_embeddings=cell_state_attribute_embeddings
 def build_lstm_zero_state(self):
  with tf.variable_scope("lstm_zero_state")as scope:
   cell_state=self.cell_state_attribute_embeddings
   hidden_state=self.hidden_state_attribute_embeddings
